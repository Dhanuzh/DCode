...
function New-SelfLockProcess([int]$processId) {
  return [pscustomobject]@{ name = 'DCode installer'; pid = $processId }
}
...
try {
  ...
  if ($targetPathFull -and (Test-Path -LiteralPath $targetPathFull -PathType Leaf)) {
    ...
    $knownRelative = @(
      ...
      'resources\bundled-dcode-core\win32-x64\dcode-core.exe'
    )
    $known = @(
      $knownRelative |
...
namespace DCode.RestartManager {
...
'@
  ...
  $result = [DCode.RestartManager.Native]::RmStartSession([ref]$sessionHandle, 0, $key)
  if ($result -ne 0) {
  ...
  try {
    for ($i = 0; $i -lt $resources.Count; $i += 256) {
      ...
      $result = [DCode.RestartManager.Native]::RmRegisterResources($sessionHandle, [uint32]$chunk.Count, $chunk, 0, [IntPtr]::Zero, 0, $null)
      if ($result -ne 0) {
    ...
    for ($attempt = 0; $attempt -lt 6; $attempt++) {
      ...
      $count = [uint32]0
      $reasons = [uint32]0
      $result = [DCode.RestartManager.Native]::RmGetList($sessionHandle, [ref]$needed, [ref]$count, $null, [ref]$reasons)
    ...
    if ($result -eq $ERROR_MORE_DATA -or $needed -gt 0) {
      for ($attempt = 0; $attempt -lt 6; $attempt++) {
        ...
        $count = $needed
        $apps = New-Object 'DCode.RestartManager.RM_PROCESS_INFO[]' $count
        $result = [DCode.RestartManager.Native]::RmGetList($sessionHandle, [ref]$needed, [ref]$count, $apps, [ref]$reasons)
    ...
    [void][DCode.RestartManager.Native]::RmEndSession($sessionHandle)
  }
} catch {
