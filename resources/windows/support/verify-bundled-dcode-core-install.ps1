...
function Write-VerifyLog {
  ...
  $payload = [ordered]@{
    ...
    updated = $false
    instDir = $InstallDir
    event = 'verify-bundled-dcode-core'
    message = $Message
  }
...
function Test-BundledResourcesOnce {
  ...
  $baseDir = Join-Path $resourcesDir "bundled-dcode-core\$RuntimeKey"

  if (-not (Test-Directory $failures 'dcode-core' '' $baseDir)) {
    return $failures
  }

  Test-NonEmptyFile $failures 'dcode-core' '' (Join-Path $baseDir 'dcode-core.exe') $true $baseDir | Out-Null

  $bundleManifestPath = Join-Path $baseDir 'manifest.json'
  if (Test-NonEmptyFile $failures 'dcode-core-manifest' '' $bundleManifestPath $false $baseDir) {
    $bundleManifest = Read-JsonFile $bundleManifestPath
    if (-not $bundleManifest) {
      $failures.Add((New-Failure 'publish_or_install_missing' 'dcode-core-manifest' '' $bundleManifestPath 'invalid_json')) | Out-Null
    } else {
      if ($bundleManifest.platform -ne $expectedPlatform) {
        $failures.Add((New-Failure 'publish_or_install_missing' 'dcode-core-manifest' '' $bundleManifestPath "platform_mismatch:$($bundleManifest.platform)")) | Out-Null
      }
      if ($bundleManifest.arch -ne $expectedArch) {
        $failures.Add((New-Failure 'publish_or_install_missing' 'dcode-core-manifest' '' $bundleManifestPath "arch_mismatch:$($bundleManifest.arch)")) | Out-Null
      }
    }
  }
...
for ($attempt = 1; $attempt -le 5; $attempt++) {
  ...
  if ($failures.Count -eq 0) {
    Write-VerifyLog "verify-bundled-dcode-core result=ok runtime=$RuntimeKey attempts=$attempt"
    exit 0
  }

  $summary = ($failures | ConvertTo-Json -Compress -Depth 5)
  if ($attempt -lt 5) {
    Write-VerifyLog "verify-bundled-dcode-core result=retry classification=resource_pending_landing runtime=$RuntimeKey attempt=$attempt failures=$summary"
    Start-Sleep -Milliseconds 500
  } else {
    Write-VerifyLog "verify-bundled-dcode-core result=fail runtime=$RuntimeKey failures=$summary"
  }
}
