    ...
    $$installerSelfLock = (Test-AionUiSamePath $$currentOutDir $$targetPath) -or (Test-AionUiSamePath $$currentOutDir $$instDir); \
      ...
      try { \
        ...
        if ($$targetPath -and (Test-Path -LiteralPath $$targetPath -PathType Leaf)) { \
          ...
          $$knownRelative = @('${AIONUI_APP_EXECUTABLE_FILENAME}', '${UNINSTALL_FILENAME}', 'resources\app.asar', 'resources\app-update.yml', 'resources\bundled-dcode-core\win32-x64\dcode-core.exe'); \
        ...
        if ($$resources.Count -eq 0) { \
          if ($$installerSelfLock -and $$installerPid -gt 0) { \
            $$lockerText = 'DCode installer(' + $$installerPid + ')'; \
            [System.IO.File]::WriteAllText($$lockerListPath, $$lockerText, (New-Object System.Text.UTF8Encoding $$false)); \
            $$selfLockers = @([pscustomobject]@{ name = 'DCode installer'; pid = [int]$$installerPid }); \
        ...
        if ($$lockers.Count -eq 0 -and $$installerSelfLock -and $$installerPid -gt 0) { $$lockers = @([pscustomobject]@{ name = 'DCode installer'; pid = [int]$$installerPid }) }; \
  ...
  ${If} $AionUiIsUpdated == "1"
    ${If} ${Silent}
      nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "& { \
        ...
        $$appDir = Join-Path $$env:APPDATA 'DCode'; \
