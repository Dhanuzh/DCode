...
!define AIONUI_APP_EXECUTABLE_FILENAME "DCode.exe"
...
!macro AIONUI_LOG_EXTRACT_RESULT _METHOD
  ${IfNot} ${FileExists} "$INSTDIR\DCode.exe"
    !insertmacro AIONUI_FAIL_UX \
      "${AIONUI_E_EXTRACT_FAILED}" \
      "event=extract result=fail method=${_METHOD} missing=DCode.exe" \
      ...
      "extract result=fail method=${_METHOD} missing=DCode.exe instDir=$INSTDIR" \
      "extract result=fail method=${_METHOD} missing=DCode.exe instDir=$INSTDIR"
  ${Else}
