...
!macro AIONUI_VERIFY_CORE_APP_FILES
  ...
  !insertmacro AIONUI_VERIFY_REQUIRED_FILE "$INSTDIR\DCode.exe" "DCode.exe"
...
!macro AIONUI_VERIFY_BUNDLED_AIONCORE_RESOURCES _RUNTIME_KEY
  InitPluginsDir
  File "/oname=$PLUGINSDIR\verify-bundled-dcode-core-install.ps1" "${PROJECT_DIR}\resources\windows\support\verify-bundled-dcode-core-install.ps1"
  nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -File "$PLUGINSDIR\verify-bundled-dcode-core-install.ps1" -InstallDir "$INSTDIR" -RuntimeKey "${_RUNTIME_KEY}" -LogPath "$AionUiSessionLogPath"`
  Pop $AionUiVerifyResourceResult

  ${If} $AionUiVerifyResourceResult != 0
    !insertmacro AIONUI_FAIL_UX \
      "${AIONUI_E_BUNDLED_AIONCORE_INCOMPLETE}" \
      "event=session-end result=fail code=${AIONUI_E_BUNDLED_AIONCORE_INCOMPLETE} detail=bundled-dcode-core-incomplete runtime=${_RUNTIME_KEY} result=$AionUiVerifyResourceResult" \
      ...
      "bundled-dcode-core-incomplete runtime=${_RUNTIME_KEY} result=$AionUiVerifyResourceResult instDir=$INSTDIR" \
      "bundled-dcode-core-incomplete runtime=${_RUNTIME_KEY} result=$AionUiVerifyResourceResult instDir=$INSTDIR"
  ${EndIf}
!macroend
