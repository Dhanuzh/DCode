<p align="center">
  <img src="./resources/aionui-banner-1.png" alt="DCode - Cowork with AI Agents" width="100%">
</p>
...
<p align="center">
  <a href="https://x.com/AionUi/status/2079493379914961069" target="_blank">
    <img src="./resources/kimi/contributor-bonus-black-blue.jpg" alt="DCode × Kimi Contributor Bonus" width="600">
  </a>
</p>

<p align="center">
  🎁 <strong>DCode × Kimi Partnership</strong>: Free premium <strong>Kimi "Allegretto" plans ($39/mo · ¥199/mo value)</strong> for our contributors!
</p>
...
[Why Choose DCode?](#-why-choose-dcode-over-claude-cowork) ·
...
**DCode is more than a chat client.** It's a Cowork platform where AI agents work alongside you on your computer — reading files, writing code, browsing the web, and automating tasks. You see everything the agent does, and you're always in control.

|                                 | Traditional AI Chat Clients | **DCode (Cowork)**                                                                                                                      |
| :------------------------------ | :-------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------- |
...
<p align="center">
  <img src="./resources/offica-ai BANNER-function.png" alt="DCode Cowork Platform" width="800">
</p>
...
DCode ships with a complete AI agent engine. Unlike tools that require you to install CLI agents separately, **DCode works the moment you install it**.
...
If you already use Claude Code, Codex, Hermes Agent, or OpenClaw, DCode auto-detects them and lets you Cowork with all of them — alongside the built-in agent.
...
- **MCP Unified Management** — manage MCP (Model Context Protocol) tools in one place; DCode injects or syncs compatible transports according to each agent's capabilities
...
- **Parallel multi-agent execution** — Leader breaks tasks into subtasks and delegates to Teammate agents running in parallel; each Teammate uses its own model through the selected backend. External agents connect through ACP (Agent Client Protocol), while DCode coordinates the team
...
Other AI apps give you a chatbox with your API key. **DCode gives you a full Cowork agent.**
...
Same agent capabilities — file read/write, web search, image generation, tool use — regardless of which model powers it. DCode supports **30+ AI platforms** including cloud services and local deployments.
...
DCode also supports [NewAPI](https://github.com/QuantumNous/new-api) gateway service — a unified AI model hub that aggregates and distributes various LLMs. Flexibly switch between different models in the same interface to meet various task requirements.
...
- **Three-tier Skills** — Builtin skills (shipped with DCode), custom skills (your own), and Extension skills (contributed by third-party extensions); enable/disable per conversation with the skill indicator
...
DCode supports three skill layers: **built-in** skills (shipped with the app), **custom** skills (user-defined), and **extension** skills (loaded from the Extension SDK).
...
DCode includes **21 professional assistants** with predefined capabilities, extendable through custom skills:
...
- **🧭 DCode Butler** — Product guidance, setup help, and troubleshooting inside DCode

**Custom Skills**: Import or create skills from **Settings → Skills**, then attach them to assistants as needed. Skills come from three sources: built-in (shipped with DCode), custom (your own), and Extension (contributed via the Extension SDK). Built-in skills include `pptx`, `docx`, `pdf`, `xlsx`, `mermaid`, and more.

> 💡 The canonical built-in definitions live in AionCore's [assistant catalog](https://github.com/iOfficeAI/AionCore/blob/main/crates/aionui-app/assets/builtin-assistants/assistants.json); assistant rules remain Markdown and can be customized in DCode.
...
_Your 24/7 AI assistant — access DCode from any device, anywhere._
...
> **Setup:** DCode Settings → WebUI Settings → Channel, configure the Bot Token.
...
- **Keep-awake** — DCode automatically prevents system sleep while tasks are active, and detects missed triggers after wake
...
## 🤔 Why Choose DCode Over Claude Cowork?
...
DCode is a **free and open-source Multi-AI Agent Desktop**. Compared to Claude Cowork which only runs on macOS and is locked to Claude, DCode is its full-model, cross-platform enhanced version.

| Dimension     | Claude Cowork | DCode                                                    |
| :------------ | :------------ | :-------------------------------------------------------- |
...
A: <strong>No.</strong> DCode has a built-in AI agent that works immediately after installation. Just enter any API key to get started. If you also have CLI tools like Claude Code or Gemini CLI installed, DCode will auto-detect and integrate them for even more capabilities.
</details>

<details>
<summary><strong>Q: What can I do with DCode?</strong></summary>
A: DCode is your <strong>private Cowork workspace</strong>. The built-in agent can batch organize folders, process Excel data, generate documents, search the web, and generate images. With Multi-Agent Mode, you can also leverage Claude Code, Codex, and other powerful CLI agents through the same interface.
</details>

<details>
<summary><strong>Q: Is it free?</strong></summary>
A: DCode is completely free and open source. You only pay for the API usage of whichever provider you choose, and you can use API keys from any provider you prefer.
</details>

<details>
<summary><strong>Q: Can I run DCode on a server (headless)?</strong></summary>
A: Yes — DCode WebUI mode runs as a standalone HTTP server. See the WebUI section above for setup instructions.
</details>
...
> **Made a video about DCode?** [Let us know on X](https://x.com/AionUi) and we'll feature it here!
...
DCode local development uses two repositories: AionUi for the Electron frontend and AionCore for the local backend. See the [Development Guide](docs/contributing/development.md) for the full macOS, Linux, and Windows setup.
