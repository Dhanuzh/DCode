# DCode WebUI Mode - Startup Guide

DCode supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts DCode with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use DCode from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\DCode\DCode.exe" --webui

# Or if DCode is in your PATH
DCode.exe --webui
```
...
```bash
# Using full path
/Applications/DCode.app/Contents/MacOS/DCode --webui

# Or using open command
open -a DCode --args --webui
```

### Method 2: Create Shell Script

Create `start-dcode-webui.sh`:
...
```bash
chmod +x start-dcode-webui.sh
./start-dcode-webui.sh
```
...
chmod +x DCode-*.AppImage

# Run with --webui flag
./DCode-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/dcode-webui.desktop`:

```ini
[Desktop Entry]
Name=DCode WebUI
Comment=Start DCode in WebUI mode
Exec=/opt/DCode/DCode --webui
Icon=DCode
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/dcode-webui.desktop
```
...
Create `~/bin/start-dcode-webui.sh`:
...
```bash
chmod +x ~/bin/start-dcode-webui.sh
```
...
Create `/etc/systemd/system/dcode-webui.service`:

```ini
[Unit]
Description=DCode WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/DCode/DCode --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable dcode-webui.service
sudo systemctl start dcode-webui.service

# Check status
sudo systemctl status dcode-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run DCode in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making DCode accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running DCode WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
...
wget https://github.com/iOfficeAI/AionUi/releases/download/vVERSION/DCode-VERSION-linux-arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/iOfficeAI/AionUi/releases/download/v1.5.2/DCode-1.5.2-linux-arm64.deb

# Install the package
apt install -y ./DCode-*.deb

# Verify installation
which DCode
```

#### 5. Launch DCode WebUI

```bash
# Start DCode in WebUI mode with no-sandbox flag
DCode --no-sandbox --webui
```
...
To access DCode from other devices on your local network:

```bash
# Start with --remote flag
DCode --no-sandbox --webui --remote
...
DCode --no-sandbox --webui --port 8080
```
...
chmod +x /opt/DCode/DCode
```

#### Out of Memory

DCode requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if DCode is running: look for "Server started" message
...
4. **Keep device charged** - Running DCode consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **DCode Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch DCode quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-dcode.sh << 'EOF'
#!/bin/bash
echo "Starting DCode WebUI..."
DCode --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-dcode.sh
...
```bash
proot-distro login ubuntu -- bash -c "DCode --no-sandbox --webui --remote"
```
...
```cmd
DCode.exe --webui --remote
```

### macOS

```bash
/Applications/DCode.app/Contents/MacOS/DCode --webui --remote
```
...
netsh advfirewall firewall add rule name="DCode WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add DCode
...
which DCode
# or
find /opt -name "DCode" 2>/dev/null
```
...
```powershell
& "C:\Program Files\DCode\DCode.exe" --webui 2>&1 | Tee-Object -FilePath dcode.log
```

**macOS/Linux:**

```bash
/path/to/DCode --webui 2>&1 | tee dcode.log
```
...
export DCODE_PORT=8080

# Allow remote access without passing --remote
export DCODE_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as DCODE_ALLOW_REMOTE=true)
export DCODE_HOST=0.0.0.0

# Then start the application
DCode --webui

# You can also pass the port directly via CLI
DCode --webui --port 8080
```
...
| Platform | Location                                                 |
| -------- | -------------------------------------------------------- |
| Windows  | `%APPDATA%/DCode/webui.config.json`                     |
| macOS    | `~/Library/Application Support/DCode/webui.config.json` |
| Linux    | `~/.config/DCode/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\DCode\DCode.exe" --resetpass

# Or for a specific user
"C:\Program Files\DCode\DCode.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/DCode.app/Contents/MacOS/DCode --resetpass

# Or for a specific user
/Applications/DCode.app/Contents/MacOS/DCode --resetpass username
```
...
# Or for a specific user
DCode --resetpass username

# Or using full path
/opt/DCode/DCode --resetpass
```
...
**Happy using DCode in WebUI mode!** 🚀
