...
DCode development uses two repositories:

- **AionCore** (`https://github.com/iOfficeAI/AionCore.git`) builds the local backend binary: `dcode-core` on macOS/Linux and `dcode-core.exe` on Windows.
...
# Verify that DCode will be able to find the backend.
which dcode-core
dcode-core --help
```

If `which dcode-core` prints nothing, add `export PATH="$HOME/.cargo/bin:$PATH"` to your shell profile (`~/.zshrc`, `~/.bashrc`, or your shell's equivalent), open a new terminal, and verify again.
...
# Verify that DCode will be able to find the backend.
where.exe dcode-core
dcode-core --help
```

If `where.exe dcode-core` prints nothing, make sure `%USERPROFILE%\.cargo\bin` is in your user `Path`, open a new PowerShell window, and verify again.

### 3. Start AionUi

Run these commands from the `AionUi` repository in a terminal where `dcode-core` is discoverable.
...
During startup, DCode launches `dcode-core` automatically and passes the backend port to the renderer. You do not need to start AionCore in a separate terminal.
...
### `Cannot find "dcode-core" binary`

DCode cannot find the backend from the `PATH` inherited by `bun run start`.

Check from the same terminal where you start DCode:
...
If the command fails, add Cargo's binary directory to `PATH` and start DCode from a new terminal.

### `dcode-core` Works in a Terminal but DCode Still Cannot Find It

Make sure you start `bun run start` from the same terminal environment that can run `dcode-core --help`. IDE terminals and GUI-launched shells can inherit a different `PATH`; restart the IDE or launch it from a terminal after updating `PATH`.

### Backend Changes Do Not Show Up

Quit DCode, reinstall AionCore with `cargo install --path crates/aionui-app --locked --force`, then start DCode again. The Electron app owns the backend subprocess during development, so a running DCode instance will not pick up a newly installed binary until it restarts.
...
This sets `DCODE_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`DCode-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.dcode-dev-2`, `~/.dcode-config-dev-2`)
...
DCode uses **electron-vite** for fast bundling:
