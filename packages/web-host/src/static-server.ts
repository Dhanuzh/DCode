 ...
 * /login and /logout to dcode-core. All auth goes to backend's dcode-auth crate;
 * /login and /logout are dcode-auth's top-level paths, the rest live under
...
export async function startStaticServer(opts: StaticServerOptions): Promise<StaticServerHandle> {
  ...
  const http_server: Server = http.createServer(async (req, res) => {
    try {
      ...
      // /login and /logout are dcode-auth's top-level auth endpoints: proxy them too
