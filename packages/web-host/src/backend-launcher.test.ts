...
describe('buildSpawnArgs', () => {
  ...
  it('passes prompt dump flag in development only when DCODE_DUMP_PROMPTS is enabled', () => {
    const prev = process.env.DCODE_DUMP_PROMPTS;
    process.env.DCODE_DUMP_PROMPTS = '1';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.DCODE_DUMP_PROMPTS;
      else process.env.DCODE_DUMP_PROMPTS = prev;
    }
  });
  ...
  it('respects DCODE_LOG_LEVEL override', () => {
    const prev = process.env.DCODE_LOG_LEVEL;
    process.env.DCODE_LOG_LEVEL = 'trace';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.DCODE_LOG_LEVEL;
      else process.env.DCODE_LOG_LEVEL = prev;
    }
  });
});

describe('buildSpawnEnv', () => {
  it('merges process.env with DCODE_* dir vars', () => {
    const env = buildSpawnEnv({
      cacheDir: '/c',
      workDir: '/w',
      logDir: '/l',
    });
    expect(env.DCODE_CACHE_DIR).toBe('/c');
    expect(env.DCODE_WORK_DIR).toBe('/w');
    expect(env.DCODE_LOG_DIR).toBe('/l');
...
describe('findAvailablePort', () => {
  ...
  it('skips ports blocked by Fetch so health checks can use the selected port', async () => {
    ...
    try {
      ...
      expect(infoSpy).toHaveBeenCalledWith('[dcode-core] skipped fetch-blocked backend port 1720');
      expect(infoSpy).toHaveBeenCalledWith('[dcode-core] selected backend port 40404 after 2 attempts');
    } finally {
      infoSpy.mockRestore();
    }
  });
...
describe('BackendLifecycleManager.start (success path)', () => {
  it('lets dcode-core choose the backend port and waits for the reported listening event', async () => {
    vi.mocked(createServer).mockImplementation(() => {
    ...
    const mgr = new BackendLifecycleManager(APP_META_PACKAGED, () => '/abs/path/dcode-core');
  ...
  it('spawns with correct args, waits for /health, reports running', async () => {
    ...
    const resolveBackend = vi.fn(() => '/abs/path/dcode-core');
    ...
    try {
      ...
      expect(spawnCall[0]).toBe('/abs/path/dcode-core');
      ...
      expect(opts.env.DCODE_CACHE_DIR).toBe('/c');
      expect(opts.env.DCODE_WORK_DIR).toBe('/w');
      expect(opts.env.DCODE_LOG_DIR).toBe('/l');
      ...
      expect(infoSpy).toHaveBeenCalledWith(
        expect.stringContaining('[dcode-core] health ready on port 55555 after 1 attempts, elapsed_ms=')
      );
    } finally {
      fetchSpy.mockRestore();
      infoSpy.mockRestore();
...
describe('BackendLifecycleManager.start (health timeout)', () => {
  ...
  it('captures backend boundary code and stage from early-exit stderr', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.open databasePath=/db/path/dcode-backend.db: failed to initialize application data\n'
      )
    );
  ...
  it('captures backend boundary code when stderr drains after exit but before close', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.migration databasePath=/db/path/dcode-backend.db: failed to initialize application data\n'
      )
    );
    child.emit('close', 1, null);
  ...
  it('kills child and reports listen_timeout when dcode-core never reports a port', async () => {
    vi.useFakeTimers();
  ...
  it('includes startup diagnostics when health check times out', async () => {
    ...
    const expectedRejection = expect(startPromise).rejects.toMatchObject({
      ...
      details: expect.objectContaining({
        ...
        binaryPath: '/abs/path/dcode-core',
        port: 33334,
...
describe('BackendLifecycleManager crash restart', () => {
  ...
  it('logs crash restart scheduling details', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[dcode-core] child exited unexpectedly; scheduling restart', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 65303,
      restartCount: 1,
  ...
  it('logs when crash restart limit is exceeded', () => {
    ...
    expect(mgr.status).toBe('error');
    expect(errorSpy).toHaveBeenCalledWith('[dcode-core] child exited unexpectedly; restart limit exceeded', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 0,
      restartCount: 4,
...
function makePeerAlreadyRunningError(): BackendStartupError {
  return new BackendStartupError('dcode-core exited before health check passed', {
    stage: 'early_exit',
...
describe('BackendLifecycleManager.start peer retry', () => {
  it('retries with bounded backoff and succeeds once the peer releases the data dir', async () => {
    vi.useFakeTimers();
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const mgr = new BackendLifecycleManager(APP_META, () => '/abs/path/dcode-core');
    ...
    const started = mgr.start('/data/dcode-backend.db');
  ...
  it('does not retry a non-peer startup failure', async () => {
    ...
    await expect(mgr.start('/data/dcode-backend.db')).rejects.toBe(nonPeerError);
