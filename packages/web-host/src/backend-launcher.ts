/**
 * Lifecycle manager for the dcode-core subprocess (web-host version).
 *
...
export type BackendLaunchOptions = {
  ...
  logDir?: string;
  /**
   * System dirs exposed to the backend via DCODE_{CACHE,WORK,LOG}_DIR env.
...
export class BackendStartupCancelledError extends Error {
  constructor(message = 'dcode-core startup cancelled') {
    super(message);
...
export function buildSpawnArgs(config: SpawnConfig): string[] {
  const logLevel = process.env.DCODE_LOG_LEVEL || (config.isPackaged ? 'info' : 'debug');
  const args = [
    '--port',
    String(config.port),
    '--data-dir',
  ...
  if (!config.isPackaged && process.env.DCODE_DUMP_PROMPTS === '1') args.push('--dump-prompts');
...
/**
 * Backend reads DCODE_{CACHE,WORK,LOG}_DIR env vars to report system dirs
 * (see AionCore/crates/dcode-system/src/sysinfo.rs). Inject them so the
 * backend's `/api/system/info` matches what Electron main persists in
 * ProcessEnv('dcode.dir').
 */
export function buildSpawnEnv(dirs: BackendDirConfig): NodeJS.ProcessEnv {
  return {
    ...process.env,
    DCODE_CACHE_DIR: dirs.cacheDir,
    DCODE_WORK_DIR: dirs.workDir,
    DCODE_LOG_DIR: dirs.logDir,
  };
}
...
// Bare, payload-less readiness marker emitted by dcode-core once `axum::serve`
...
// Benign boundary code emitted by an dcode-core instance that yielded the
...
export function findAvailablePort(
  ...
  if (preferredPort && firstRequestedPort === 0) {
    console.info(`[dcode-core] skipped fetch-blocked backend port ${preferredPort}`);
  }
      ...
      server.listen(requestedPort, '127.0.0.1', () => {
        ...
        server.close(() => {
          ...
          if (resolvedPort > 0 && !isFetchForbiddenPort(resolvedPort)) {
            console.info(`[dcode-core] selected backend port ${resolvedPort} after ${attempt} attempts`);
            resolve(resolvedPort);
            return;
          }
          if (resolvedPort > 0 && remainingAttempts > 1) {
            console.info(`[dcode-core] skipped fetch-blocked backend port ${resolvedPort}`);
...
export class BackendLifecycleManager {
  ...
  async start(
    ...
    // Bounded retry loop for the transient "a peer dcode-core already owns this
    ...
    for (let attempt = 0; attempt < PEER_RETRY_MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        console.warn(
          `[dcode-core] a peer already owns the data directory; retrying startup in ${backoff}ms (attempt ${attempt + 1}/${PEER_RETRY_MAX_ATTEMPTS})`
        );
    ...
    throw lastPeerError ?? new Error('dcode-core startup failed after peer retries');
  }
  ...
  private async attemptStart(
    ...
    try {
      ...
      throw new BackendStartupError(
        'dcode-core startup failed while resolving backend binary',
        {
    ...
    console.log(`[dcode-core] starting: ${binaryPath} ${args.join(' ')}`);
    ...
    try {
      ...
      this._status = 'error';
      throw makeStartupError('spawn', 'dcode-core startup directory preparation failed', error);
    }
    ...
    try {
      ...
      this._status = 'error';
      throw makeStartupError('spawn', 'dcode-core process spawn threw before startup', error);
    }
    ...
    const startupFailure = new Promise<never>((_resolve, reject) => {
      ...
      this.childProcess?.once('error', (error) => {
        ...
        this._status = 'error';
        rejectOnce(makeStartupError('spawn_error', 'dcode-core process emitted an error before startup', error));
      });
      ...
      this.childProcess?.once('close', (code, signal) => {
        ...
        if (!pendingStartupExit.startupSettledAtExit) {
          if (pendingStartupExit.statusAtExit === 'stopped') {
            rejectOnce(new BackendStartupCancelledError('dcode-core startup cancelled before health check passed'));
            return;
          }
          rejectOnce(
            makeStartupError('early_exit', 'dcode-core exited before health check passed', undefined, {
        ...
        if (pendingStartupExit.statusAtExit === 'starting') {
          void Promise.resolve(
            options?.onPendingExit?.(
              makeStartupError('early_exit', 'dcode-core exited after startup health timeout', undefined, {
                exitCode: exitCode ?? undefined,
                signal: exitSignal ?? undefined,
              })
            )
          ).catch((error) => {
            console.error('[dcode-core] pending exit handler failed:', error);
          });
        }
      });
    });
    ...
    const reportedPort = new Promise<number>((resolve, reject) => {
      ...
      reportedPortTimer = setTimeout(() => {
        rejectReportedPort(
          makeStartupError('listen_timeout', 'dcode-core did not report its listening port before timeout', undefined, {
    ...
    this.childProcess.stdout?.on('data', (data: Buffer) => {
      ...
      for (const line of data.toString().split('\n')) {
        ...
        if (trimmed) console.log(`[dcode-core] ${line}`);
      }
    });
    ...
    this.childProcess.stderr?.on('data', (data: Buffer) => {
      ...
      for (const line of data.toString().split('\n')) {
        if (line.trim()) console.error(`[dcode-core] ${line}`);
      }
    });
    ...
    if (healthOrReady.kind === 'health' && !healthOrReady.health.ok) {
      ...
      const healthTimeoutError = makeStartupError(
        'health_timeout',
        'dcode-core failed to start within timeout',
        undefined,
        {
      ...
      if (keptAlive && this.childProcess) {
        startupSettled = true;
        console.warn(`[dcode-core] health check timed out; keeping process alive on port ${this._port}`);
        void Promise.resolve(options?.onHealthTimeout?.(healthTimeoutError)).catch((error) => {
          console.error('[dcode-core] health timeout handler failed:', error);
        });
    ...
    if (healthOrReady.kind === 'ready') {
      console.info(
        `[dcode-core] ready signal received on port ${this._port}, elapsed_ms=${Date.now() - startupStartedAt}, data-dir: ${dbPath}`
      );
    } else {
      console.info(
        `[dcode-core] health ready on port ${this._port} after ${healthOrReady.health.diagnostics.healthCheckAttempts} attempts, elapsed_ms=${healthOrReady.health.diagnostics.healthCheckElapsedMs}, data-dir: ${dbPath}`
      );
    }
    return this._port;
  }
  ...
  private continueWaitingForHealth(
    ...
    void (async () => {
      ...
      console.info(
        `[dcode-core] late ${outcome.kind === 'ready' ? 'ready signal' : 'health ready'} on port ${port}, elapsed_ms=${elapsedMs}, data-dir: ${this._lastDbPath}`
      );
      await onReady?.(port);
    })().catch((error) => {
      console.error('[dcode-core] background health wait failed:', error);
    });
  }
  ...
  private handleCrash(code: number | null, signal?: NodeJS.Signals | string | null): void {
    ...
    if (this.restartCount > this.maxRestarts) {
      this._status = 'error';
      console.error('[dcode-core] child exited unexpectedly; restart limit exceeded', crashContext);
      return;
    }

    const delay = Math.pow(2, this.restartCount - 1) * 1000;
    console.warn('[dcode-core] child exited unexpectedly; scheduling restart', {
      ...crashContext,
      delayMs: delay,
    });
    ...
    setTimeout(() => {
      ...
      this.start(this._lastDbPath, this._lastLogDir, this._lastDirs, this._lastOptions, this._port)
        ...
        .catch((error) => {
          this._status = 'error';
          console.error('[dcode-core] restart after crash failed', {
            port: this._port,
            restartCount: this.restartCount,
            maxRestarts: this.maxRestarts,
            delayMs: delay,
