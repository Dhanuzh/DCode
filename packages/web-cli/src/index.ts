import { startWebHost, startStaticServer } from '@dcode/web-host';
import type { WebHostHandle, StaticServerHandle } from '@dcode/web-host';
...
// tarball layout:
//   dcode-web/
//   ├── dcode-web              ← bun-compiled standalone binary (process.execPath)
//   ├── package.json             ← for runtime version lookup
//   ├── bundled-dcode-core/<plat-arch>/dcode-core[.exe]
//   └── static/                  ← SPA assets
//
...
const isPackaged = (() => {
  ...
  return exeName === 'dcode-web' || exeName === 'dcode-web.exe';
})();

const BACKEND_BINARY = process.platform === 'win32' ? 'dcode-core.exe' : 'dcode-core';
const DEFAULT_PORT = 25808;
const RESET_COMMAND = isPackaged ? 'dcode-web resetpass' : 'bun run resetpass';
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const envOverride = process.env.DCODE_BACKEND_BIN;
  if (envOverride) return path.resolve(envOverride);
  const platArch = `${process.platform}-${process.arch}`;
  const bundled = path.join(cliRoot, 'bundled-dcode-core', platArch, BACKEND_BINARY);
  return bundled;
}
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.DCODE_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.dcode-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.DCODE_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolvePort(flags: Map<string, string | true>): number {
  ...
  const env = process.env.DCODE_PORT ?? process.env.PORT;
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.DCODE_ALLOW_REMOTE ?? process.env.DCODE_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[dcode-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[dcode-web] version    : ${version}`);
  console.log(`[dcode-web] data dir   : ${dataDir}`);
  console.log(`[dcode-web] log dir    : ${logDir}`);
  console.log(`[dcode-web] static dir : ${staticDir}`);
  console.log(`[dcode-web] backend bin: ${backendBin}`);
  console.log(`[dcode-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download dcode-core and set DCODE_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('DCode WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[dcode-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[dcode-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
    ...
    console.log('');
    console.log('DCode WebUI is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[dcode-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[dcode-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `dcode-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[dcode-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set DCODE_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[dcode-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[dcode-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[dcode-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[dcode-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[dcode-web] username: ${username}`);
    console.log(`[dcode-web] new password: ${newPassword}`);
    console.log('[dcode-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: dcode-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.dcode-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.dcode-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  DCODE_PORT, DCODE_ALLOW_REMOTE, DCODE_DATA_DIR, DCODE_LOG_DIR,
  DCODE_BACKEND_BIN, DCODE_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: dcode-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[dcode-web] fatal:', err.message);
