...
export type EnsureAdminPasswordOptions = {
   ...
   * by launch context — packaged tarball = `dcode-web resetpass`, in-repo dev
...
export async function ensureAdminPassword(
  ...
  const resetCmd = opts.resetCommand ?? 'dcode-web resetpass';
  ...
  try {
    ...
    deps.warn(`[dcode-web] could not verify admin credentials: ${err instanceof Error ? err.message : String(err)}`);
    return;
  }
  ...
  if (!needsSetup) {
    ...
    deps.log(`[dcode-web] Log in with username "${username}". Forgot the password? Run \`${resetCmd}\`.`);
    return;
  }
  ...
  try {
    ...
    if (!resetRes.ok) {
      deps.warn(`[dcode-web] /api/webui/reset-password returned ${resetRes.status} — run \`${resetCmd}\``);
      return;
    }
    ...
    if (!newPassword) {
      deps.warn(`[dcode-web] /api/webui/reset-password returned no new_password — run \`${resetCmd}\``);
      return;
    }
    const username = await fetchAdminUsername(deps, opts.backendPort);
    deps.log(`[dcode-web] Generated initial admin password: ${newPassword}`);
    deps.log(`[dcode-web] Log in with username "${username}" and change it from the UI.`);
  } catch (err) {
    deps.warn(
      `[dcode-web] failed to seed initial admin password: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
