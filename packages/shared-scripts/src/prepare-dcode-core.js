/**
 * Prepare the dcode-core backend binary for packaging.
 *
 * Resolution order:
 *  1. GitHub Actions artifact download when DCODE_BACKEND_RUN_ID is set
 *  2. GitHub release download (requires version or defaults to "latest")
 *  3. Complete local bundle from DCODE_BACKEND_LOCAL_BUNDLE_DIR
 *  4. Local binary fallback from DCODE_BACKEND_LOCAL_BINARY
 *
 * Output: {projectRoot}/resources/bundled-dcode-core/{platform}-{arch}/
 *   - dcode-core[.exe]
 *   - manifest.json
 *   - managed-resources/...
 *
 * NOTE: the GitHub-download branches (1-2) still match `aioncore`-named
 * release assets — that is the literal filename the upstream iOfficeAI/AionCore
 * project publishes, not our own naming, and this repo does not currently
 * publish its own dcode-core releases for that path to target instead. Those
 * branches are unused by this project's actual build (which builds and bundles
 * the backend locally); only the local-bundle/local-binary output naming below
 * has been renamed.
 *
 * @module prepare-dcode-core
 */
...
const { verifyBundledAioncoreResources } = require('./verify-bundled-dcode-core-resources');
...
function prepareManagedResources(binaryPath, targetDir) {
  ...
  execFileSync(binaryPath, ['--data-dir', dataDir, 'prepare-managed-resources', '--bundle-out', bundleOut], {
    ...
    env: {
      ...process.env,
      DCODE_BUNDLED_MANAGED_RESOURCES: '',
    },
  });
...
function prepareAioncore(options) {
  ...
  const actionsRunId = (process.env.DCODE_BACKEND_RUN_ID || '').trim();
  ...
  const targetDir = path.join(projectRoot, 'resources', 'bundled-dcode-core', runtimeKey);
  const binaryName = getBinaryName(platform);
  ...
  const localBundleDir = (process.env.DCODE_BACKEND_LOCAL_BUNDLE_DIR || '').trim();
  if (localBundleDir) {
  ...
  if (!sourcePath) {
    const localBinary = (process.env.DCODE_BACKEND_LOCAL_BINARY || '').trim();
    if (localBinary) {
  ...
  if (sourcePath) {
    ...
    verifyPreparedAioncoreBundle(projectRoot, platform, arch);
    console.log(
      `  Bundled dcode-core prepared: resources/bundled-dcode-core/${runtimeKey}/${binaryName} [source=${sourceType}]`
    );
