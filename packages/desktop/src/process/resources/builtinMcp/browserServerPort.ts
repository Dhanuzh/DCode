 ...
 * 只认自己进程树继承下来的端口。
 *
 * 端口由 Electron 主进程写进 DCODE_CDP_ACTIVE_PORT，经 dcode-core 继承到这里，所以
 ...
 * writes it into DCODE_CDP_ACTIVE_PORT and dcode-core passes it down, so failing to
...
export const resolveBrowserUrl = (deps: ResolveBrowserUrlDeps): string | null => {
  ...
  const rawPort = env.DCODE_CDP_ACTIVE_PORT?.trim();
  if (rawPort) {
...
export const resolveBridgeToken = (deps: ResolveBrowserUrlDeps): string | null => {
  const raw = deps.env.DCODE_CDP_BRIDGE_TOKEN?.trim();
  return raw ? raw : null;
};
