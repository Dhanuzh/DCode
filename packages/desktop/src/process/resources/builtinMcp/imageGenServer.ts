...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env.DCODE_IMG_PLATFORM;
  const base_url = process.env.DCODE_IMG_BASE_URL;
  const api_key = process.env.DCODE_IMG_API_KEY;
  const model = process.env.DCODE_IMG_MODEL;
...
IMPORTANT: When user provides multiple images, ALWAYS pass ALL images to the image_uris parameter as an array.`,
    ...
    async ({ prompt, image_uris }) => {
      ...
      const proxy = process.env.DCODE_IMG_PROXY || undefined;
