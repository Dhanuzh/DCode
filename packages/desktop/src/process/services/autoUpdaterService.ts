...
const FORCE_DEV_AUTO_UPDATE_ENV = 'DCODE_FORCE_DEV_AUTO_UPDATE';
...
class AutoUpdaterService extends EventEmitter {
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        'provider: generic',
        `url: ${cdnFeedOptions.url}`,
        'updaterCacheDirName: com.dcode.app',
        '',
      ].join('\n');
  ...
  private moveCwdOutOfInstallDirForWindowsHandoff(): void {
    ...
    try {
      const safeCwd = path.join(app.getPath('temp'), 'dcode-updater-cwd');
   ...
   * while the downloaded archive contains the packaged app (com.dcode.app). Squirrel.Mac
