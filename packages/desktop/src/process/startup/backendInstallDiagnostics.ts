...
const BUNDLED_DCODE_CORE_DIR = 'bundled-dcode-core';
...
export function collectBackendInstallDiagnostics(
  ...
  const bundledDirPath = resourcesPath ? pathApi.join(resourcesPath, BUNDLED_DCODE_CORE_DIR) : undefined;
  const runtimeDirPath =
    resourcesPath && runtimeKey ? pathApi.join(resourcesPath, BUNDLED_DCODE_CORE_DIR, runtimeKey) : undefined;
  const binaryPath =
