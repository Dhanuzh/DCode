...
function classifyIncompleteInstallation(details: ErrorWithDetails['details']): BackendStartupFailureInfo | undefined {
  ...
  const missingBundledDcodeCoreDir = !resourcesDirEntries.includes('bundled-dcode-core/');
  const missingRuntimeDir = details.runtimeDirExists === false && typeof details.runtimeKey === 'string';
  const missingResources = missingBundledDcodeCoreDir ? ['bundled-dcode-core/'] : [];
  if (details.runtimeDirExists === false && typeof details.runtimeKey === 'string') {
    missingResources.push(`bundled-dcode-core/${details.runtimeKey}/`);
  }
  ...
  if (missingManagedResourcesDir && typeof details.runtimeKey === 'string') {
    missingResources.push(`bundled-dcode-core/${details.runtimeKey}/managed-resources/`);
  }
  ...
  if (missingRuntimeBinary && typeof details.runtimeKey === 'string' && typeof details.binaryName === 'string') {
    missingResources.push(`bundled-dcode-core/${details.runtimeKey}/${details.binaryName}`);
  }
  ...
  return {
    incompleteInstallationKind:
      missingBundledDcodeCoreDir || missingRuntimeDir || missingManagedResourcesDir
        ? 'missing_directory_resources'
        : 'missing_backend_binary',
    missingBackendBinary:
      missingBundledDcodeCoreDir || missingRuntimeDir || missingManagedResourcesDir || missingRuntimeBinary,
    missingBundledDcodeCoreDir,
...
// A transient concurrent-startup race (two dcode-core instances briefly bootstrapping
