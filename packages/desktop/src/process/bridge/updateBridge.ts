...
const DEFAULT_REPO = 'iOfficeAI/DCode';
const DEFAULT_USER_AGENT = 'DCode';
const ALLOWED_ASSET_EXTS = new Set(['.exe', '.msi', '.dmg', '.zip', '.deb', '.rpm']);
const CDN_HOST = 'static.dcode.com';
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = process.env.DCODE_GITHUB_REPO?.trim();
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `DCode-update-${Date.now()}`;
};
