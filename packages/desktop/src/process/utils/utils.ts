...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'dcode');
};
 ...
 * Release builds use ~/.dcode; dev builds use ~/.dcode-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.dcode，Dev 模式使用 ~/.dcode-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'dcode');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.dcode'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.dcode-config; dev builds use ~/.dcode-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.dcode-config，Dev 模式使用 ~/.dcode-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.dcode-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[DCode] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[DCode] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[DCode] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[DCode] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
