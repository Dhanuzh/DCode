...
import { startWebHost, type WebHostHandle } from '@dcode/web-host';
...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.DCODE_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.DCODE_ALLOW_REMOTE || process.env.DCODE_REMOTE);
  const hostHint = process.env.DCODE_HOST?.trim();
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns ~/.dcode[-dev] symlink on macOS to sidestep
