...
const STORAGE_PATH = {
  config: 'dcode-config.txt',
  chatMessage: 'dcode-chat-message.txt',
  chat: 'dcode-chat.txt',
  env: '.dcode-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[DCode] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[DCode] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[DCode] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('dcode.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'dcode-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'dcode-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'dcode-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'dcode-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[DCode] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `crates/dcode-app/assets/builtin-assistants/` and
 * `crates/dcode-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[DCode:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog (see dcode-assistant crate) and
  ...
  try {
    ...
    console.error('[DCode] Failed to ensure assistant dirs:', error);
  }
