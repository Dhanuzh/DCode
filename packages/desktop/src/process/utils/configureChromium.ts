...
// BEFORE any getPath() call so the whole data tree (config, dcode-core DB, logs)
...
// migration. Guarded by DCODE_E2E_TEST so it never affects dev/production.
// 仅 E2E：把 userData 指向一次性沙箱目录，避免测试读写真实数据库。
const e2eUserDataDir = process.env.DCODE_E2E_TEST === '1' ? process.env.DCODE_E2E_USER_DATA_DIR : undefined;
...
//
// The 9230-9250 reservation and the ~/.dcode-cdp-registry.json instance registry were
...
//
// Override via DCODE_CDP_PORT env variable. Set to "0" or "false" to disable.
// ---------------------------------------------------------------------------
 ...
 * 顺手删掉遗留的实例注册表文件。
 *
 * 这个文件（~/.dcode-cdp-registry.json）以前记录「每个实例占了哪个 CDP 端口」。相关逻辑
 * 已随应用级 remote-debugging-port 一起删除，但升级上来的机器上文件还在，里面是一堆早已
 * 无效的 pid/端口。留着只会让人以为还有这套机制，所以清掉。best-effort，失败无所谓。
 *
 * Remove the leftover instance-registry file. ~/.dcode-cdp-registry.json used to record
...
function removeLegacyCdpRegistryFile(): void {
  try {
    const legacyRegistry = path.join(os.homedir(), '.dcode-cdp-registry.json');
 ...
 * 用户看不见的 Chrome）。
 *
 * DCODE_CDP_PORT 现在只当开关用（"0"/"false" 关闭，其它非空值开启）。它的数值不再
 ...
 *
 * DCODE_CDP_PORT now acts purely as a switch ("0"/"false" disables, any other non-empty
...
function shouldEnableCdp(config: CdpConfig): boolean {
  const envVal = process.env.DCODE_CDP_PORT;
