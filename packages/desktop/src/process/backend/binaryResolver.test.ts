...
describe('resolveBinaryPath', () => {
  ...
  it('attaches bundled path diagnostics when dcode-core cannot be resolved', () => {
    const resourcesPath = '/app/resources';
    const runtimeKey = `${process.platform}-${process.arch}`;
    const binaryName = process.platform === 'win32' ? 'dcode-core.exe' : 'dcode-core';
    const bundledDir = join(resourcesPath, 'bundled-dcode-core');
    ...
    vi.mocked(readdirSync).mockImplementation((path) => {
      if (path === resourcesPath) return [dirEntry('bundled-dcode-core', true)];
    ...
    expect(() => resolveBinaryPath()).toThrow('Cannot find "dcode-core" binary');
    ...
    try {
      ...
      expect(error).toMatchObject({
        ...
        diagnostics: expect.objectContaining({
          ...
          resourcesDirEntries: ['bundled-dcode-core/'],
          runtimeDirEntries: ['manifest.json'],
          pathLookupCommand: process.platform === 'win32' ? 'where dcode-core' : 'which dcode-core',
