...
import { BackendLifecycleManager } from '@dcode/web-host';
...
import { startWebHost } from '@dcode/web-host';
...
const isE2ETestMode = process.env.DCODE_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.DCODE_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[DCode] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[DCode] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[DCode] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[DCode] runBackendMigrations completed');
    } catch (error) {
      console.error('[DCode] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[DCode] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  const reason = process.env.DCODE_DEBUG_BACKEND_STARTUP_FAILURE as BackendStartupFailureInfo['reason'] | undefined;
  if (!reason) {
    return null;
  }
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[DCode] Ignoring DCODE_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[DCode] Ignoring unknown DCODE_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[DCode] Creating main window...');
  ...
  console.log(`[DCode] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[DCode] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[DCode] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[DCode] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  const disableAutoUpdater =
    process.env.DCODE_DISABLE_AUTO_UPDATE === '1' || process.env.DCODE_E2E_TEST === '1' || isCiRuntime;
  if (!disableAutoUpdater) {
    Promise.all([import('./process/services/autoUpdaterService'), import('./process/bridge/updateBridge')])
      .then(([{ autoUpdaterService }, { createAutoUpdateStatusBroadcast }]) => {
        ...
        // build: DCode's final version guides users to the website instead of
    ...
    console.log('[DCode] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[DCode] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[DCode] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[DCode] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[DCode] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[DCode] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[DCode] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[DCode] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[DCode] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[DCode] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[DCode] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[DCode] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[DCode] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[DCode:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
   ...
   *
   * 两个值靠进程继承链传给 Agent：dcode-core 是本进程的子进程，内置浏览器 MCP 又是
   * dcode-core 的子进程，所以不用落库、不用写配置。但继承是「spawn 那一刻的快照」：
   * backend-launcher 用 `{ ...process.env }` spawn dcode-core，此后我们再改 process.env
   * 对已经起来的 dcode-core 毫无影响。一旦这段挪到 backend 启动之后，dcode-core 继承到的
   ...
   * style. Both values reach the agent by process inheritance (dcode-core is our child; the
   * in-app browser MCP is dcode-core's child), so neither is persisted. But inheritance is a
   * snapshot taken at spawn time: backend-launcher spawns dcode-core with
   * `{ ...process.env }`, and any later mutation of our process.env is invisible to the
   * already-running dcode-core. Move this block after backend startup and dcode-core inherits
  ...
  if (cdpStartupEnabled) {
    try {
      ...
      process.env.DCODE_CDP_ACTIVE_PORT = String(bridge.port);
      process.env.DCODE_CDP_BRIDGE_TOKEN = bridge.token;
  ...
  if (debugBackendStartupFailure) {
    ...
    // Start dcode-core only after initializeProcess(). initStorage may open
  ...
  try {
    ...
    console.error('[DCode] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[DCode] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`DCode --webui` or packaged `dcode-web` mode that
      ...
      const handle = await startWebHost({
        ...
        // Expose the same DCODE_{CACHE,WORK,LOG}_DIR env the desktop IPC path
        ...
        backend: {
          ...
          port: (() => {
            ...
            if (!port) {
              throw new Error('[WebUI] Cannot start: dcode-core is not running (globalThis.__backendPort unset)');
            }
            return port;
          })(),
        },
...
// Register dcode:// as the default protocol client
...
// macOS: handle dcode:// URLs via the open-url event
...
// instance lock. A lock-losing instance must NOT spawn a competing dcode-core
// backend — doing so races the first instance's dcode-core over the same data
...
if (shouldRegisterBackendStartup(gotTheLock)) {
  void app
    ...
    .catch((error) => {
      ...
      console.error('[DCode] App initialization failed:', error);
      app.quit();
    });
}
...
installQuitCleanup({
  ...
  // Stop dcode-core subprocess — backend shutdown kills all agent children
...
app.on('will-quit', () => {
  console.log('[DCode] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[DCode] quit (exitCode=${exitCode})`);
});
