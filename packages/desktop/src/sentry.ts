...
function isBackendStartupFailureEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.['dcode.failure'] === 'backend_startup';
}

function isUserFeedbackEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.type === 'user-feedback' || event.tags?.['dcode.installation_integrity.user_report'] === 'true';
}
...
function getInstallPathKind(resourcesPath: unknown): string | undefined {
  ...
  if (normalized.includes('\\appdata\\local\\programs\\dcode\\resources')) {
    return 'user_local_programs';
  }
  if (
    normalized.includes('\\program files\\dcode\\resources') ||
    normalized.includes('\\program files (x86)\\dcode\\resources')
  ) {
...
export async function captureBackendStartupFailure(error: unknown): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.setTag('dcode.failure', 'backend_startup');
    scope.setTag('dcode.backend_startup.reason', failureInfo.reason);
    if (failureInfo.runtime) {
      scope.setTag('dcode.backend_startup.runtime', failureInfo.runtime);
    }
    if (failureInfo.packageArch) {
      scope.setTag('dcode.backend_startup.package_arch', failureInfo.packageArch);
    }
    if (failureInfo.deviceArch) {
      scope.setTag('dcode.backend_startup.device_arch', failureInfo.deviceArch);
    }
    if (failureInfo.expectedDownloadArch) {
      scope.setTag('dcode.backend_startup.expected_download_arch', failureInfo.expectedDownloadArch);
    }
    if (typeof failureInfo.isRosettaTranslated === 'boolean') {
      scope.setTag('dcode.backend_startup.rosetta_translated', getBooleanTagValue(failureInfo.isRosettaTranslated));
    }
    if (typeof details?.stage === 'string') {
      scope.setTag('dcode.backend_startup.stage', details.stage);
    }
    if (failureInfo.backendBoundaryCode) {
      scope.setTag('dcode.backend_startup.boundary_code', failureInfo.backendBoundaryCode);
    }
    if (failureInfo.backendBoundaryStage) {
      scope.setTag('dcode.backend_startup.boundary_stage', failureInfo.backendBoundaryStage);
    }
    if (failureInfo.localDataIssueKind) {
      scope.setTag('dcode.backend_startup.local_data_issue_kind', failureInfo.localDataIssueKind);
    }
    if (failureInfo.incompleteInstallationKind) {
      scope.setTag('dcode.backend_startup.incomplete_installation_kind', failureInfo.incompleteInstallationKind);
    }
    for (const [tag, value] of [
      ['dcode.backend_startup.missing_bundled_dir', getBooleanTagValue(failureInfo.missingBundledDcodeCoreDir)],
      ['dcode.backend_startup.missing_runtime_dir', getBooleanTagValue(failureInfo.missingRuntimeDir)],
      ['dcode.backend_startup.missing_binary', getBooleanTagValue(failureInfo.missingBackendBinary)],
      ['dcode.backend_startup.missing_hub_dir', getBooleanTagValue(failureInfo.missingHubDir)],
      ['dcode.backend_startup.missing_pet_states_dir', getBooleanTagValue(failureInfo.missingPetStatesDir)],
      ['dcode.backend_startup.missing_pwa_dir', getBooleanTagValue(failureInfo.missingPwaDir)],
      ['dcode.backend_startup.install_path_kind', getInstallPathKind(details?.resourcesPath)],
      ['dcode.backend_startup.last_update_status', getString(autoUpdateDiagnostics?.lastEvent?.status)],
      [
        'dcode.backend_startup.health_polling_delayed',
        getBooleanTagValue(
          typeof details?.healthCheckPollingDelayed === 'boolean' ? details.healthCheckPollingDelayed : undefined
        ),
      ],
      ['dcode.backend_startup.health_attempts_bucket', getHealthAttemptBucket(details?.healthCheckAttempts)],
      [
        'dcode.backend_startup.health_attempt_deficit_bucket',
        getHealthAttemptBucket(details?.healthCheckAttemptDeficit),
      ],
      ['dcode.backend_startup.health_timeout_overrun_bucket', getDurationBucket(details?.healthCheckTimeoutOverrunMs)],
      ['dcode.backend_startup.health_max_attempt_gap_bucket', getDurationBucket(details?.healthCheckMaxAttemptGapMs)],
      [
        'dcode.backend_startup.seconds_since_quit_and_install',
    ...
    if (details) {
      scope.setContext('dcode-core_startup', details);
      scope.setExtra('dcode-core_startup', details);
    }
    scope.setContext('dcode-core_startup_classification', { ...failureInfo });
    scope.setExtra('dcode-core_startup_classification', failureInfo);
    scope.setContext('dcode-core_install_diagnostics', installDiagnostics);
    scope.setExtra('dcode-core_install_diagnostics', installDiagnostics);
