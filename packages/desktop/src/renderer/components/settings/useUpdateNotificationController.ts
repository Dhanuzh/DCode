...
export const UPDATE_AVAILABLE_EVENT = 'dcode-update-available';
...
const RELEASES_PAGE_URL = 'https://github.com/iOfficeAI/DCode/releases';
...
export const useUpdateNotificationController = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('dcode-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('dcode-open-update-modal', handleWindowOpen);
