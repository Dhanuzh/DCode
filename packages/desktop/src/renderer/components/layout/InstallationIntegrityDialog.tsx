...
const DCODE_DOWNLOAD_URL = 'https://dcodes.site/';
...
export function openDownloadLatest(): void {
  window.open(DCODE_DOWNLOAD_URL, '_blank', 'noopener,noreferrer');
}
...
function buildInstallationIntegrityTags(diagnostics: InstallationIntegrityDiagnostics): FeedbackEventTags {
  const tags: FeedbackEventTags = {
    'dcode.installation_integrity.user_report': 'true',
    'dcode.installation_integrity.report_source': diagnostics.source,
  };

  if (diagnostics.runtime?.failureKind) {
    tags['dcode.installation_integrity.failure_kind'] = diagnostics.runtime.failureKind;
  }
  if (diagnostics.runtime?.resource) {
    tags['dcode.runtime_resource'] = diagnostics.runtime.resource;
  }
  if (diagnostics.runtime?.resourceId) {
    tags['dcode.runtime_resource_id'] = diagnostics.runtime.resourceId;
  }
  if (diagnostics.runtime?.scopeKind) {
    tags['dcode.runtime_scope'] = diagnostics.runtime.scopeKind;
  }

  const reason = diagnostics.backendStartupFailure?.reason;
  if (typeof reason === 'string') {
    tags['dcode.backend_startup_failure.reason'] = reason;
  }
  const backendBoundaryCode = diagnostics.backendStartupFailure?.backendBoundaryCode;
  if (typeof backendBoundaryCode === 'string') {
    tags['dcode.backend_startup_failure.backend_boundary_code'] = backendBoundaryCode;
  }
  const backendBoundaryStage = diagnostics.backendStartupFailure?.backendBoundaryStage;
  if (typeof backendBoundaryStage === 'string') {
    tags['dcode.backend_startup_failure.backend_boundary_stage'] = backendBoundaryStage;
  }
...
export async function reportInstallationIntegrityDiagnostics(
  ...
  if (typeof window !== 'undefined' && window.__dcodeE2ETest) {
