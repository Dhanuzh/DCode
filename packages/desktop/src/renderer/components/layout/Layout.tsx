...
const Layout: React.FC<{
  ...
  // The "DCode" wordmark acts as Home / Back-to-Chat, but only from settings routes.
  ...
  const handleBrandHome = useCallback(() => {
    ...
    try {
      target = sessionStorage.getItem('dcode:last-non-settings-path');
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      window.dispatchEvent(new CustomEvent('dcode-open-update-modal', { detail: { source: 'tray' } }));
    };
            ...
            >
              ...
              >
                ...
                {isSettingsRoute ? (
                  ...
                  <div className='text-16px text-t-primary collapsed-hidden font-semibold'>DCode</div>
                )}
                {isMobile && !collapsed && (
                  <button
                    type='button'
