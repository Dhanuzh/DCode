...
/** Backend manifest id of the built-in DCode Butler assistant. */
 ...
 * select the DCode Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the DCode Butler for you' })
            );
          }
        }
      } catch (error) {
