...
export type ScmRepository = {
   ...
   * field `is_worktree`, dcode-core `types.rs`). Only surfaced under one-level workspace
   ...
   * dcode-core `types.rs`). Omitted when the primary is outside the current view — the
 ...
 * dcode-core `types.rs`). Both fields are optional — a provider that cannot resolve
...
export type ScmStatus = {
   ...
   * to `resources`/`seq`, per dcode-core `types.rs`; `skip_serializing_if none`). It is
