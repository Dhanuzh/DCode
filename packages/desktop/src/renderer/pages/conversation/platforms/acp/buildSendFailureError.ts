...
const DCODE_TRANSPORT_ERROR_CODES = new Set([
...
export const buildSendFailureError = (error: unknown, message: string): AgentStreamErrorInfo => {
  ...
  if (isBackendHttpError(error) && DCODE_TRANSPORT_ERROR_CODES.has(error.code)) {
    return {
      message,
      code: error.code,
      ownership: 'dcode',
