...
const AddPlatformModal = ModalHOC<{
  ...
  const vertexAuthMethod = Form.useWatch('vertexAuthMethod', form);
  ...
  // Vertex is addressed by project + location and authenticates via a service
  // account or ADC, so it has no endpoint or bearer key to enter.
  const isVertex = platform === 'vertex-ai';
  const usesCloudCredentials = isBedrock || isVertex;
  ...
  useEffect(() => {
    if (modalProps.visible) {
      ...
      // `global` serves models that regional endpoints do not, so default there.
      form.setFieldValue('vertexAuthMethod', 'adc');
      form.setFieldValue('vertexLocation', 'global');
      protocolDetection.reset();
      ...
      // Pre-fill from deep link data (dcode:// protocol)
      if (deepLinkData?.base_url || deepLinkData?.api_key) {
        ...
        // Platform picker is temporarily limited to Google Cloud (Vertex AI)
        // only (see the MODEL_PLATFORMS.filter below), so default to it
        // instead of the normal DEFAULT_PLATFORM_VALUE.
        form.setFieldValue('platform', 'vertex-ai');
      }
    }
    ...
    form
      ...
      .then((values) => {
        ...
        const provider: IProvider = {
          ...
          base_url: usesCloudCredentials ? '' : values.base_url || selectedPlatform?.base_url || '',
          api_key: usesCloudCredentials ? '' : values.api_key,
        ...
        // Add Vertex configuration if platform is Google Cloud (Vertex AI)
        if (isVertex) {
          provider.vertex_config = {
            auth_method: values.vertexAuthMethod,
            project_id: values.vertexProjectId,
            location: values.vertexLocation,
            ...(values.vertexAuthMethod === 'serviceAccount'
              ? { service_account_json: values.vertexServiceAccountJson }
              : {}),
          };
        }
      ...
      <div>
        <Form form={form} layout='vertical' className='[&_.arco-form-item]:mb-12px [&_.arco-form-item:last-child]:mb-0'>
          ...
          >
            ...
            >
              {/* Temporarily limited to Google Cloud (Vertex AI) only — the other
                  preset platforms are still fully supported (MODEL_PLATFORMS is
                  untouched), just hidden from this picker for now. Remove this
                  .filter(...) to restore the full platform list. */}
              {MODEL_PLATFORMS.filter((plat) => plat.value === 'vertex-ai').map((plat) => (
          ...
          {/* Base URL - shown for every platform except those authenticating with
              cloud credentials (Bedrock, Vertex) so users can see and edit the
              endpoint. Preset platforms are prefilled with their default URL and
              offer a reset button to restore it. */}
          <Form.Item
            hidden={usesCloudCredentials}
            label={
          ...
          */}
          ...
          <Form.Item
            ...
            required={!usesCloudCredentials}
            rules={[{ required: !usesCloudCredentials }]}
            field={'api_key'}
            extra={
          ...
          {/* Google Cloud Vertex AI — one project serves both Gemini and Claude */}
          <Form.Item
            hidden={!isVertex}
            label={t('settings.vertex.projectId')}
            field={'vertexProjectId'}
            required={isVertex}
            rules={[{ required: isVertex }]}
            extra={t('settings.vertex.projectIdHint')}
          >
          ...
          <Form.Item
            hidden={!isVertex}
            label={t('settings.vertex.location')}
            field={'vertexLocation'}
            initialValue='global'
            required={isVertex}
            rules={[{ required: isVertex }]}
            extra={t('settings.vertex.locationHint')}
          >
            <Select showSearch allowCreate>
              <Select.Option value='global'>global</Select.Option>
              <Select.Option value='us-east5'>us-east5</Select.Option>
              <Select.Option value='us-central1'>us-central1</Select.Option>
              <Select.Option value='europe-west1'>europe-west1</Select.Option>
              <Select.Option value='asia-southeast1'>asia-southeast1</Select.Option>
            </Select>
          </Form.Item>

          <Form.Item
            hidden={!isVertex}
            label={t('settings.vertex.authMethod')}
            field={'vertexAuthMethod'}
            initialValue='adc'
            required={isVertex}
            rules={[{ required: isVertex }]}
          >
            <Select>
              <Select.Option value='serviceAccount'>{t('settings.vertex.authMethodServiceAccount')}</Select.Option>
              <Select.Option value='adc'>{t('settings.vertex.authMethodAdc')}</Select.Option>
            </Select>
          </Form.Item>

          <Form.Item
            hidden={!isVertex || vertexAuthMethod !== 'serviceAccount'}
            label={t('settings.vertex.serviceAccountJson')}
            field={'vertexServiceAccountJson'}
            required={isVertex && vertexAuthMethod === 'serviceAccount'}
            rules={[{ required: isVertex && vertexAuthMethod === 'serviceAccount' }]}
            extra={t('settings.vertex.serviceAccountJsonHint')}
          >
            <Input.TextArea rows={4} placeholder='{ "type": "service_account", ... }' />
          </Form.Item>

          {isVertex && vertexAuthMethod === 'adc' && (
            <div className='text-11px text-t-secondary mb-12px'>{t('settings.vertex.adcHint')}</div>
          )}
                ...
                isFullUrl ? undefined : (
                  <Search
                    onClick={async (e) => {
                      ...
                      // Vertex discovers models by probing the project, so it
                      // needs project + location rather than an endpoint or key.
                      if (isVertex) {
                        const values = form.getFields();
                        if (!values.vertexProjectId || !values.vertexLocation || !values.vertexAuthMethod) {
                          message.warning(t('settings.vertex.fillRequiredFields'));
                          return;
                        }
                        if (values.vertexAuthMethod === 'serviceAccount' && !values.vertexServiceAccountJson) {
                          message.warning(t('settings.vertex.fillRequiredFields'));
                          return;
                        }
                        const vertex_config = {
