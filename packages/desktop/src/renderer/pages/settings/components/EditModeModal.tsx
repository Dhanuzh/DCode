...
const EditModeModal = ModalHOC<{ data?: IProvider; onChange(data: IProvider): void }>(
  ({ modalProps, modalCtrl, ...props }) => {
    ...
    // service account or ADC, so it hides base_url/api_key like Bedrock does.
    const vertexAuthMethod = Form.useWatch('vertexAuthMethod', form);
    const isVertex = data?.platform === 'vertex-ai';
    ...
    useEffect(() => {
      if (data) {
        form.setFieldsValue({
          ...
          vertexAuthMethod: data.vertex_config?.auth_method || 'adc',
          vertexProjectId: data.vertex_config?.project_id || '',
          // `global` carries models that regional endpoints do not, so it is
          // the safer default.
          vertexLocation: data.vertex_config?.location || 'global',
          vertexServiceAccountJson: data.vertex_config?.service_account_json || '',
        });
      }
    }, [data, form]);
        ...
        <div>
          <Form form={form} layout='vertical'>
            ...
            <Form.Item
              ...
              required={data?.platform !== 'gemini' && data?.platform !== 'gemini-vertex-ai' && !usesCloudCredentials}
              rules={[
                {
                  required:
                    data?.platform !== 'gemini' && data?.platform !== 'gemini-vertex-ai' && !usesCloudCredentials,
                },
              ]}
              field={'base_url'}
            >
            ...
            {/* Google Cloud Vertex AI — project identity */}
            <Form.Item
              hidden={!isVertex}
            ...
            <Form.Item hidden={!isVertex || vertexAuthMethod !== 'adc'} label={' '}>
              <div className='text-11px text-t-secondary'>{t('settings.vertex.adcHint')}</div>
            </Form.Item>
