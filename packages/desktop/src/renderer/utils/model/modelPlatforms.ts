 ...
 * Platform type
 */
export type PlatformType = 'gemini' | 'gemini-vertex-ai' | 'vertex-ai' | 'anthropic' | 'custom' | 'new-api' | 'bedrock';
...
export const MODEL_PLATFORMS: PlatformConfig[] = [
  ...
  // One Google Cloud project serves both Gemini and Claude, so this platform
  // exposes them together instead of splitting them across providers. Model
  // availability is per-project, so the backend probes rather than guesses.
  {
