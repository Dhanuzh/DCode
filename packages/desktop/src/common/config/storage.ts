 ...
 * 会话来源类型 - 标识会话创建的来源
 */
export type ConversationSource = 'dcode' | 'telegram' | 'lark' | 'dingtalk' | 'weixin' | 'wecom' | (string & {});
...
interface IChatConversation<T, Extra> {
  ...
  /** 会话来源，默认为 dcode / Conversation source, defaults to dcode */
...
export type TChatConversation =
  ...
  // AionCore/crates/dcode-common/src/enums.rs and factory.rs.
...
export interface IProvider {
   ...
   * Only used when platform is 'vertex-ai'. One project serves both Gemini
   * and Claude, and availability varies by location, so the deployment is
   * identified by project + location rather than by a bearer key.
   */
  vertex_config?: {
    auth_method: 'serviceAccount' | 'adc';
    project_id: string;
    location: string;
...
export interface IMcpServer {
  ...
  /** Built-in MCP server managed by DCode (hide edit/delete in UI) */
  builtin?: boolean;
}
...
export const BUILTIN_IMAGE_GEN_NAME = 'dcode-image-generation';
export const BUILTIN_IMAGE_GEN_LEGACY_NAMES = ['DCode Image Generation', BUILTIN_IMAGE_GEN_ID] as const;
