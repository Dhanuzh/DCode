# Package OfficeCLI Skill as DCode Assistant

Convert an OfficeCLI skill into a fully wired DCode assistant preset, or update an existing one.
...
3. Copy all files, but apply these transformations to SKILL.md:
   - **Remove version comments**: Delete any `# officecli: vX.X.X` line from inside the frontmatter. DCode's frontmatter parser (`/^---\s*\n([\s\S]*?)\n---/`) requires clean YAML — version tracking comments break parsing and skills won't appear in the Skills Center.
