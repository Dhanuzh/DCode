...
function main() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'dcode-self-lock-'));
  const installDir = path.join(root, 'install-dir');
  mkdirSync(installDir, { recursive: true });
  writeFileSync(path.join(installDir, 'existing-file.txt'), 'self-lock smoke\n', 'utf8');

  const nsiPath = path.join(root, 'dcode-self-lock-smoke.nsi');
  const exePath = path.join(root, 'dcode-self-lock-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `dcode-installer-self-lock-${new Date()
      .toISOString()
      .replace(/[-:]/g, '')
      .replace(/\..+$/, '')
      .replace('T', '-')}-log.jsonl`
  );
  const resultPath = path.join(process.env.TEMP || tmpdir(), `dcode-installer-self-lock-${process.pid}-result.txt`);
  const processControlPath = path.join(repoRoot, 'resources', 'windows', 'installer-process-control.nsh');

  const nsi = `
Unicode true
Name "DCode Installer Self Lock Smoke"
OutFile "${nsisQuote(exePath)}"
RequestExecutionLevel user
SilentInstall silent
!define VERSION "self-lock-smoke"
!define DCODE_TARGET_ARCH "x64"
!define DCODE_FALLBACK_LOG "dcode-installer-self-lock-fallback.log"
...
Section
  ...
  StrCpy $DCodeSessionId "selflock"
  StrCpy $DCodeIsUpdated "1"
  StrCpy $DCodeSessionLogPath "${nsisQuote(logPath)}"
  StrCpy $ResultFile "${nsisQuote(resultPath)}"
  InitPluginsDir
  SetOutPath $INSTDIR
  StrCpy $DCodeCurrentOutDir "$INSTDIR"
  !insertmacro DCODE_QUERY_LOCKERS "$INSTDIR" $DCodeLockerResult
  FileOpen $0 "$ResultFile" w
  FileWrite $0 "$DCodeLockerResult"
  FileWrite $0 "|$DCodeCurrentOutDir|$DCodeSessionLogPath"
  FileClose $0
  \${If} $DCodeLockerResult != 0
    SetErrorLevel 10
    Quit
  \${EndIf}
SectionEnd
`;
  ...
  try {
    ...
    if (!blocking.some((process) => process.name === 'DCode installer' && Number(process.pid) > 0)) {
      throw new Error(`expected DCode installer blocker, got ${JSON.stringify(blocking)}`);
    }
