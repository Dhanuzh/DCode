...
function resolvePackagedApp(projectRoot) {
  ...
  if (process.platform === 'win32') {
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'DCode.exe');
    ...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'DCode');
...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('DCode.exe');
    await killProcessByName('DCode');
  ...
  const env = {
    ...process.env,
    DCODE_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] DCODE_EXTENSIONS_PATH: ${env.DCODE_EXTENSIONS_PATH}`);
