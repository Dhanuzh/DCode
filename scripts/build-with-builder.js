...
/**
 * Simplified build script for DCode
...
const DEBUG_AUTO_UPDATE_CURRENT_VERSION_ENV = 'DCODE_DEBUG_AUTO_UPDATE_CURRENT_VERSION';
...
function patchElectronBuilderNsisInstaller() {
  ...
  const bundledUninstallerOverride = [
    '  ${if} ${FileExists} "$PLUGINSDIR\\DCode-fixed-uninstaller.exe"',
    '    DetailPrint `DCode-bundled-uninstaller override source.`',
    '    StrCpy $uninstallerFileName "$PLUGINSDIR\\DCode-fixed-uninstaller.exe"',
    '  ${endIf}',
  ].join('\n');
...
function writeGeneratedSentryDsnInclude(projectRoot) {
  ...
  fs.writeFileSync(
    generatedInclude,
    `!define DCODE_SENTRY_DSN "${escapeNsisDefineValue(process.env.SENTRY_DSN || '')}"\n`
  );
}
...
try {
  ...
  // 5. Prepare dcode-core binary (for packaged runtime usage)
  const { prepareAioncore } = require('../packages/shared-scripts/src/prepare-dcode-core.js');
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('DCode.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running DCode/Electron process. Attempting to close...');
        killWindowsProcesses(['DCode.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running DCode/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'DCode.exe');
    ...
    console.log('⚠️  Windows local build failed after DCode.exe was produced.');
    if (firstError) {
