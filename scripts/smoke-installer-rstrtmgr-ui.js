...
function main() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'dcode-rm-ui-'));
  const installDir = path.join(root, 'install-dir');
  mkdirSync(installDir, { recursive: true });
  const lockedFile = path.join(installDir, 'locked-by-smoke.txt');
  writeFileSync(lockedFile, 'DCode Restart Manager UI smoke lock\n', 'utf8');

  let locker = null;
  const nsiPath = path.join(root, 'dcode-rstrtmgr-ui-smoke.nsi');
  const exePath = path.join(root, 'dcode-rstrtmgr-ui-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `dcode-installer-smoke-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}.log`
  );
  ...
  const nsi = `
Unicode true
Name "DCode Restart Manager UI Smoke"
OutFile "${nsisQuote(exePath)}"
RequestExecutionLevel user
SilentInstall normal
!define DCODE_FALLBACK_LOG "dcode-installer-smoke-fallback.log"
!define VERSION "rstrtmgr-ui-smoke"
!define DCODE_TARGET_ARCH "x64"
!define DCODE_APP_EXECUTABLE_FILENAME "DCode.exe"
!define UNINSTALL_FILENAME "Uninstall DCode.exe"
!define PROJECT_DIR "${nsisQuote(repoRoot)}"
!include LogicLib.nsh
!include "${nsisQuote(messagesPath)}"
!include "${nsisQuote(processControlPath)}"

Var DCodeSessionLogPath
Var DCodeSessionId
Var DCodeIsUpdated
...
Section
  ...
  StrCpy $DCodeSessionId "rstrtmgrui"
  StrCpy $DCodeIsUpdated "1"
  InitPluginsDir
  BringToFront

  dcode_query_lockers:
    !insertmacro DCODE_QUERY_LOCKERS "${nsisQuote(lockedFile)}" $DCodeLockerResult
    StrCpy $DCodeLockerList ""
    ClearErrors
    SetDetailsPrint none
    FileOpen $DCodeLockerListFile "$PLUGINSDIR\\dcode-rm-lockers.txt" r
    \${IfNot} \${Errors}
      FileRead $DCodeLockerListFile $DCodeLockerList
      FileClose $DCodeLockerListFile
    \${EndIf}
    SetDetailsPrint lastused
    \${If} $DCodeLockerList == ""
      StrCpy $DCodeLockerList "\${DCODE_MSG_UNKNOWN_PROCESS_EN}"
      StrCpy $DCodeLockerListZh "\${DCODE_MSG_UNKNOWN_PROCESS_ZH}"
      StrCpy $DCodeLockerListEn "\${DCODE_MSG_UNKNOWN_PROCESS_EN}"
    \${Else}
      StrCpy $DCodeLockerListZh "$DCodeLockerList"
      StrCpy $DCodeLockerListEn "$DCodeLockerList"
    \${EndIf}
    MessageBox MB_RETRYCANCEL|MB_ICONEXCLAMATION "\${DCODE_MSG_FILE_OR_FOLDER_IN_USE_ZH}$\\r$\\n${nsisQuote(lockedFile)}$\\r$\\n$\\r$\\n\${DCODE_MSG_APPLICATION_USING_IT_ZH}$\\r$\\n$DCodeLockerListZh$\\r$\\n$\\r$\\n\${DCODE_MSG_CLOSE_LISTED_RETRY_ZH}$\\r$\\n$\\r$\\n\${DCODE_MSG_INSTALLER_LOG_ZH}:$\\r$\\n$DCodeSessionLogPath$\\r$\\n$\\r$\\n\${DCODE_MSG_BLOCK_SEPARATOR}$\\r$\\n$\\r$\\n\${DCODE_MSG_FILE_OR_FOLDER_IN_USE_EN}$\\r$\\n${nsisQuote(lockedFile)}$\\r$\\n$\\r$\\n\${DCODE_MSG_APPLICATION_USING_IT_EN}$\\r$\\n$DCodeLockerListEn$\\r$\\n$\\r$\\n\${DCODE_MSG_CLOSE_LISTED_RETRY_EN}$\\r$\\n$\\r$\\n\${DCODE_MSG_INSTALLER_LOG_EN}:$\\r$\\n$DCodeSessionLogPath" /SD IDCANCEL IDRETRY dcode_query_lockers
SectionEnd
`;
