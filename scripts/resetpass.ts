 ...
 *   2. No webui running → spawn a short-lived dcode-core against the same
 ...
 *   DCODE_DATA_DIR=/x bun run resetpass
...
import { startBackend, stopBackend } from '@dcode/web-host';
 ...
 * See the comment there for why the default is `~/.dcode-web*` (not `~/.dcode*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `DCODE_PORT` to match
 ...
 * pass --port / DCODE_PORT if they launched webui on a non-default one.
 */
