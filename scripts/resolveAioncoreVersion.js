 ...
 *
 * Order:
 *   1. DCODE_BACKEND_VERSION env (ad-hoc override, e.g. CI dispatch input)
 *   2. "dcodeCoreVersion" field in repo-root package.json (the pin)
...
function resolveAioncoreVersion(projectRoot) {
  const envOverride = process.env.DCODE_BACKEND_VERSION;
  ...
  try {
    ...
    if (pkg && typeof pkg.dcodeCoreVersion === 'string' && pkg.dcodeCoreVersion.trim()) {
      return pkg.dcodeCoreVersion.trim();
    }
  } catch {
    // fall through
  }
