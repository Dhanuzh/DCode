#!/usr/bin/env bash
# ============================================================================
# DCode WebUI — One-Click Installation Script
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Dhanuzh/DCode/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/dcode-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/dcode-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/Dhanuzh/DCode/releases/download}"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║     DCode WebUI Installer (No Electron)     ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/dcode-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="dcode-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
resolve_version() {
    ...
    if [[ "$VERSION" == "latest" || "$VERSION" =~ [a-zA-Z_] ]]; then
        ...
        if command -v curl &>/dev/null; then
            VERSION=$(curl -fsSL "https://api.github.com/repos/Dhanuzh/DCode/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        elif command -v wget &>/dev/null; then
            VERSION=$(wget -qO- "https://api.github.com/repos/Dhanuzh/DCode/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        else
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/Dhanuzh/DCode/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is dcode-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/dcode-web" ]]; then
        mv "${extract_temp}/dcode-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing dcode-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/dcode-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/dcode-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/dcode-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/dcode-web"
    local target_path="${INSTALL_DIR}/dcode-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by dcode-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 DCode WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/dcode-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start DCode WebUI"
        echo "    dcode-web start"
        echo ""
        echo "    # Check version"
        echo "    dcode-web version"
    else
        echo "    # Start DCode WebUI (using full path)"
        echo "    ${INSTALL_DIR}/dcode-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/dcode-web ~/.local/bin/dcode-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/Dhanuzh/DCode"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/Dhanuzh/DCode/issues"
    echo ""
