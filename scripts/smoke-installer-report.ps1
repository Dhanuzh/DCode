...
$logPath = Join-Path $env:TEMP "dcode-installer-smoke-$runId-log.jsonl"
$statusPath = Join-Path $env:TEMP 'dcode-installer-report.json'

Remove-Item -LiteralPath $statusPath -Force -ErrorAction SilentlyContinue

@(
  '{"schemaVersion":1,"event":"session-begin","session":"smoke","version":"2.1.28"}'
  '{"schemaVersion":1,"event":"failure","session":"smoke","version":"2.1.28","updated":true,"code":"E1003","phase":"atomic-failed","failedPath":"C:\\Users\\huang\\AppData\\Local\\Programs\\DCode","blockingProcesses":[],"fallbackReason":"restart-manager-no-process","message":"Windows did not identify a specific locking process. Close terminals, editors, and file managers opened in the install folder."}'
...
- File or folder: C:\Users\huang\AppData\Local\Programs\DCode
...
if ($status.copyText -notlike '*DCode installer failure E1003*') {
  throw 'copyText does not include failure heading'
}
if ($status.copyText -notlike '*To DCode Team*') {
