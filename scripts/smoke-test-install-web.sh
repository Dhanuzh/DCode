...
export INSTALL_DIR="/tmp/dcode-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/dcode-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/dcode-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/dcode-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/dcode-web"
    exit 1
fi
...
VERSION_OUTPUT=$(dcode-web version 2>&1 || echo "")
