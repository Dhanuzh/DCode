 ...
 * parsing the electron-log file for [DCode:ready] / [DCode:init] /
 * [DCode:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  // Parsed from [DCode:init] marks
  initTotalMs: number;
  // Parsed from [DCode:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'DCode-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'DCode', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'DCode', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'DCode', 'logs', `${today}.log`));
  }
...
// Matches: [DCode:ready] <label> +<ms>ms
// Matches: [DCode:init]  <label> +<ms>ms
// Matches: [DCode:process] <label> +<ms>ms
const MARK_REGEX = /\[DCode:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[DCode] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[DCode] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[DCode] Showing main window')) marks.logs.showingMainWindow = true;
  }
