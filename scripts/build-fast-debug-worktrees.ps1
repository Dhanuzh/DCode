param(
  ...
  [string]$WorktreeRoot = (Join-Path $PSScriptRoot '..\..\dcode-build-worktrees'),
...
function Wait-BuildProcess($Build, [int]$TimeoutSeconds) {
  ...
  $source = Join-Path $Build.worktreePath "out\DCode-$($Build.version)-win-x64.exe"
  ...
  $target = Join-Path $script:OutputDirResolved "DCode-$($Build.version)-win-x64.exe"
  Copy-Item -LiteralPath $source -Destination $target -Force
...
if (-not (Test-Path -LiteralPath (Join-Path $localAioncoreBundleDir 'managed-resources') -PathType Container)) {
  $localAioncoreBundleDir = Join-Path $env:LOCALAPPDATA 'Programs\DCode\resources\bundled-aioncore\win32-x64'
}
...
try {
  foreach ($version in $buildVersions) {
    $worktreePath = Join-Path $runRoot "DCode-$version"
