...
const tarballName = `dcode-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'dcode-web');
...
const executableName = platform === 'win32' ? 'dcode-web.exe' : 'dcode-web';
...
// `dcode-web version` match the tarball filename.
...
const backendSrc = path.join(projectRoot, 'resources/bundled-dcode-core', `${platform}-${arch}`);
const backendDest = path.join(tarballContentDir, 'bundled-dcode-core', `${platform}-${arch}`);
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} dcode-web`, {
