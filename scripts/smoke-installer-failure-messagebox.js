...
const INSTALLER_ERROR_SCENARIOS = [
  {
    ...
    defineName: 'DCODE_E_UNINSTALLER_COPY_OR_REBUILD_FAILED',
    code: 'E1001',
    message: 'DCode could not repair the installed uninstaller.',
    action: 'Close DCode, restart Windows if needed, then run this installer again.',
    diagnostics:
  ...
  },
  {
    ...
    defineName: 'DCODE_E_OLD_UNINSTALL_FAILED',
    code: 'E1002',
    message: 'The previous DCode uninstaller returned an error.',
    action:
  ...
  },
  {
    ...
    defineName: 'DCODE_E_INSTALL_DIR_REMOVE_OR_LOCKED',
    code: 'E1003',
    message: 'DCode could not remove or replace the previous installation directory.',
    action: 'Close DCode and any program using the install folder, then run this installer again.',
    diagnostics: 'scenario=install-dir-remove-or-locked phase=atomic-failed failedPath=install-dir',
  },
  {
    id: 'extract-failed',
    defineName: 'DCODE_E_EXTRACT_FAILED',
    code: 'E1010',
    message: 'DCode could not extract the application files correctly.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=extract-failed phase=extract method=zip missing=DCode.exe',
  },
  {
    id: 'disk-insufficient',
    defineName: 'DCODE_E_DISK_INSUFFICIENT',
    code: 'E1020',
    message: 'DCode cannot continue because the target disk does not have enough free space.',
  ...
  {
    id: 'bundled-dcode-core-incomplete',
    defineName: 'DCODE_E_BUNDLED_AIONCORE_INCOMPLETE',
    code: 'E1030',
    message: 'DCode installed, but the bundled AionCore resources are incomplete.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=bundled-dcode-core-incomplete phase=verify-bundled-dcode-core runtime=win32-x64 result=1',
  },
  {
    id: 'core-app-files-incomplete',
    defineName: 'DCODE_E_CORE_APP_FILES_INCOMPLETE',
    code: 'E1031',
    message: 'DCode installation is incomplete because a required application file is missing.',
    action: 'Reinstall DCode or download a newer installer.',
    diagnostics: 'scenario=core-app-files-incomplete phase=verify-required-file missing=resources/app.asar',
  },
  {
    id: 'arch-mismatch',
    defineName: 'DCODE_E_ARCH_MISMATCH',
    code: 'E1040',
    message: 'Installation package architecture mismatch.',
    action: 'Download the DCode installer that matches this Windows architecture, then run it again.',
    diagnostics: 'scenario=arch-mismatch phase=arch-check target=x64 actual=arm64',
  },
  {
    id: 'active-installer-conflict',
    defineName: 'DCODE_E_ACTIVE_INSTALLER_CONFLICT',
    code: 'E1050',
    message: 'Another DCode installer appears to still be active.',
  ...
  },
  {
    ...
    defineName: 'DCODE_E_REGISTRY_STATE_INVALID',
    code: 'E1060',
    message: 'DCode found an invalid previous-install registry state.',
    action: 'Uninstall the old DCode from Windows Settings, then run this installer again.',
  ...
  },
  {
    ...
    defineName: 'DCODE_E_ACTIVE_MARKER_WRITE_FAILED',
    code: 'E1070',
    message: 'DCode could not write the active-installer marker.',
  ...
  },
  {
    ...
    defineName: 'DCODE_E_INVALID_INSTALL_PATH',
    code: 'E1090',
...
function copyHarnessProject(projectRoot) {
  ...
  writeFileSync(path.join(supportDir, '_sentry-dsn.generated.nsh'), '!define DCODE_SENTRY_DSN ""\n', 'utf8');
}
...
function readInstallerErrorDefinitions() {
  ...
  const definitions = Array.from(source.matchAll(/!define\s+(DCODE_E_[A-Z0-9_]+)\s+"(E\d{4})"/g), (match) => ({
    defineName: match[1],
    code: match[2],
  }));
  if (definitions.length === 0) {
    throw new Error('No DCODE_E_* installer error codes found.');
  }
  return definitions;
}
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*DCode installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "DCode installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "DCode Failure MessageBox Smoke"
OutFile "${nsisQuote(exePath)}"
RequestExecutionLevel user
SilentInstall normal
!define PROJECT_DIR "${nsisQuote(projectRoot)}"
!define VERSION "0.0.0-smoke"
!define DCODE_TARGET_ARCH "x64"
!define DCODE_RUNTIME_KEY "win32-x64"
!include LogicLib.nsh
!include nsDialogs.nsh
!include "${nsisQuote(path.join(projectRoot, 'resources', 'windows', 'installer-observability.nsh'))}"
!macro DCODE_CLEAR_ACTIVE_INSTALLER_MARKER
!macroend
!include "${nsisQuote(path.join(projectRoot, 'resources', 'windows', 'installer-errors-sentry.nsh'))}"

Section
  StrCpy $INSTDIR "$TEMP\\DCode-messagebox-smoke"
  StrCpy $DCodeSessionId "smokembox-${nsisQuote(scenario.code)}"
  StrCpy $DCodeIsUpdated "1"
  StrCpy $DCodeSessionLogPath "${nsisQuote(logPath)}"
  BringToFront
  !insertmacro DCODE_FAIL_UX \
...
function runHarness({ autoDecline, compileOnly, makensis, scenario }) {
  const { code } = scenario;
  const root = mkdtempSync(path.join(tmpdir(), `dcode-failure-messagebox-${code}-`));
  const projectRoot = path.join(root, 'project');
  const nsiPath = path.join(root, 'dcode-failure-messagebox-smoke.nsi');
  const exePath = path.join(root, 'dcode-failure-messagebox-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `dcode-installer-messagebox-smoke-${code}-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}-log.jsonl`
  );
  const automationPath = path.join(root, 'auto-decline.ps1');
  const reportStatusPath = path.join(process.env.TEMP || tmpdir(), 'dcode-installer-report.json');
  ...
  try {
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (typeof status.copyText !== 'string' || !status.copyText.includes(`DCode installer failure ${code}`)) {
