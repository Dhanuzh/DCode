...
/**
 * @license
 * Copyright 2025 DCode (dcode.com)
 ...
 *
 * Env vars:
 *   DCODE_PORT           : static server port (default 33000)
 *   DCODE_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   DCODE_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 *   DCODE_DATA_DIR       : override userData path (default Electron-compatible)
 *   DCODE_LOG_DIR        : override log dir (default <dataDir>/logs)
 *   DCODE_STATIC_DIR     : override static dir (default out/renderer)
 *   DCODE_BACKEND_BIN    : absolute path to dcode-core binary (else PATH lookup)
 *   DCODE_BACKEND_BUNDLED_DIR : dir containing bundled-dcode-core/<plat-arch>/binary
 *   DCODE_OPEN_BROWSER   : "1"/"true" to force open, "0"/"false" to disable
 */
...
const DEFAULT_PORT = (() => {
  ...
  if (process.env.DCODE_MULTI_INSTANCE === '1') return 25810;
  return 25809;
})();
...
/**
 * Resolve the directory where dcode-core persists its SQLite DB.
 *
 * `bun run webui` runs **independently of the Electron desktop app** — it must
 * work on hosts that never installed DCode.app, and its default work dir must
 * NOT collide with Electron's.
 *
 *   --data-dir <path>       CLI override (highest priority)
 *   $DCODE_DATA_DIR        env override (same effect)
 *   otherwise               ~/.dcode-web         (production)
 *                           ~/.dcode-web-dev     (dev, default)
 *                           ~/.dcode-web-dev-2   (dev + DCODE_MULTI_INSTANCE=1)
 *
 * Why a dedicated `-web` name, not the same `~/.dcode[-dev]` that Electron
 * uses: on macOS, Electron's getDataPath() (packages/desktop/src/process/utils/
 * utils.ts) creates `~/.dcode-dev` as a **symlink** to
 * `~/Library/Application Support/DCode-Dev/dcode` so CLI tools (claude,
 ...
 * `.dcode-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.dcode-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.DCODE_DATA_DIR;
  ...
  const suffix =
    process.env.NODE_ENV === 'production' ? '' : process.env.DCODE_MULTI_INSTANCE === '1' ? '-dev-2' : '-dev';
  const dir = path.join(os.homedir(), `.dcode-web${suffix}`);
...
function resolveAllowRemote(): boolean {
  ...
  const host = process.env.DCODE_HOST?.trim();
  if (host && ['0.0.0.0', '::', '::0'].includes(host)) return true;
  return parseBoolean(process.env.DCODE_ALLOW_REMOTE ?? process.env.DCODE_REMOTE);
}

function resolveStaticDir(): string {
  if (process.env.DCODE_STATIC_DIR) return process.env.DCODE_STATIC_DIR;
  const candidate = path.join(repoRoot, 'out', 'renderer');
  if (fs.existsSync(path.join(candidate, 'index.html'))) return candidate;
  throw new Error(`Renderer assets not found at ${candidate}. Run "bun run package" first, or set DCODE_STATIC_DIR.`);
}
 ...
 *   $DCODE_NO_BUILD=1        : env-level opt-out
 *   $DCODE_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.DCODE_NO_BUILD)) return;
  if (process.env.DCODE_STATIC_DIR) return;
...
function resolveBackendBinary(): string {
  if (process.env.DCODE_BACKEND_BIN) return process.env.DCODE_BACKEND_BIN;

  const bundledBase = process.env.DCODE_BACKEND_BUNDLED_DIR ?? path.join(repoRoot, 'resources', 'bundled-dcode-core');
  ...
  throw new Error(
    `Cannot find "${BACKEND_BINARY}". Set DCODE_BACKEND_BIN, put it on PATH, or place it at ${bundled}.`
  );
}
...
async function main(): Promise<void> {
  ...
  // This keeps `bun run webui` fully self-contained on hosts without DCode.app.
  const workDir = resolveBackendDataDir();
  const staticDir = resolveStaticDir();
  const backendBin = resolveBackendBinary();
  const logDir = process.env.DCODE_LOG_DIR ?? path.join(workDir, 'logs');
